...
# 14 : 700 ms
# 18 : 900 ms
prediction_length = 10  # 10 index is 500 ms  :  prediction point
...
# hyperparams
enc_seq_len = seq_length       # input size
dec_seq_len = seq_length       # ?
output_sequence_length = 1       # output size
...
model = Transformer(dim_val, dim_attn, 1, dec_seq_len, output_sequence_length, n_decoder_layers, n_encoder_layers, n_heads).to(device)
#model.cuda()
...
test_result_data_save = {"transformer_1229_pred":test_result_data}

savemat("transformer_1229_pred.mat", test_result_data_save)
...
iter101 = 92
fig = plt.figure(figsize=(10, 5))
line1, = plt.plot(test_output_data[patient_index_test[iter101]:patient_index_test[iter101 + 1]])
line2, = plt.plot(test_result_data[patient_index_test[iter101]:patient_index_test[iter101 + 1]], 'r--')
plt.legend([line1, line2], ['True', 'Prediction'])
plt.xlabel("Time Period")
plt.ylabel("Relative patient respiratory signal")
plt.show()
