...
sys.path.extend(['/home/PycharmProjects/06_pytorch'])
...
class BiRNN(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, num_classes): #dropout
        ...
        self.lstm = nn.LSTM(input_size, hidden_size, num_layers, batch_first=True, bidirectional=True) # , dropout=1
...
for epoch in range(num_epochs):
    ...
    train_loss_mean = np.sum(train_loss_result[1:]) / (len(train_loss_result[1:])-1)
    ...
    for iter11 in range(len(patient_index_test) - 1):
        ...
        test_corr = pearsonr(test_labels[:, 0], test_predict_outputs)

        test_predict_outputs = torch.from_numpy(test_predict_outputs).float().cuda()

        test_labels = torch.from_numpy(test_labels[:, 0]).float().cuda()
...
save_path = "../06_pytorch/model_save/BiLSTM_300msec___.pth"
torch.save(model, save_path)
print('model save')
...
test_result_data_save = {"BiLSTM_300_pred":test_predict_outputs}

savemat("BiLSTM_300_pred.mat", test_result_data_save)
